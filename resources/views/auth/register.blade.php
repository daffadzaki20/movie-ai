...
<body class="bg-slate-950 text-white font-sans min-h-screen flex items-center justify-center p-6">
    ...
    <div class="glass w-full max-w-md p-8 rounded-3xl border border-slate-800 shadow-2xl transition-all duration-500 hover:scale-[1.02]">
        <h2 class="text-3xl font-bold mb-2 text-center">Buat Akun</h2>
        <p class="text-slate-400 text-center mb-8">Bergabunglah dan temukan film terbaikmu</p>
        
        <form action="/register" method="POST" class="space-y-4">
            @csrf
            <div>
                <label class="block text-sm mb-2 text-slate-300">Nama Lengkap</label>
                <input type="text" name="name" class="w-full p-4 rounded-xl bg-slate-900 border border-slate-700 focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
            </div>
            <div>
            ...
            <button class="w-full bg-indigo-600 hover:bg-indigo-500 p-4 rounded-xl font-bold transition-all transform active:scale-95">Daftar Akun</button>
        </form>
        
        <p class="text-slate-400 text-sm mt-6 text-center">Sudah punya akun? <a href="/login" class="text-indigo-400 hover:font-bold transition">Login di sini</a></p>
    </div>
</body>
<script>
    // Efek input focus
